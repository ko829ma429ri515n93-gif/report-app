...
【その他】1段落150字以上。箇条書き禁止。全て文章形式。同じ名詞の3回以上の繰り返し禁止。出力は必ず完全な日本語のみで行う。英単語・アルファベット・ローマ字を文中に混入させることを厳禁する。「for example」「such as」「e.g.」「i.e.」などの英語表現は「たとえば」「すなわち」「具体的には」に置き換える。専門用語（BCP・DXなど）は初出時のみカッコ書きで日本語説明を添える。`;
