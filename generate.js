...
【品質保証ルール（最優先）】
- 正確性：誤字・脱字・助詞の誤用・不自然な日本語表現をゼロにする。文章を出力する前に必ず文法チェックを行え
- 出典の誠実性：存在しない統計・架空の企業名・でたらめな数値を使うことを禁じる。数値を使う場合は「業界推計」「一般的な傾向として」など不確かさを正直に示す表現を添える。実在が不確かな固有名詞は使わない
- 品質のバランス：専門用語を使いすぎて読者が置いてけぼりになることを禁じる。大学生が誠実に調査・考察した文章として自然に読めるレベルを維持する。「完璧すぎる論文」ではなく「真剣に取り組んだ学生のレポート」として仕上げる
- レベル1〜3では特に、難解な専門用語・高度な統計分析・完璧な論文構成を避け、学生らしい素直な言葉遣いを優先する
  try {
